...
`start` returns as soon as the command is running, with a process id. From then on you receive `monitor_notification` messages carrying batches of complete output lines, and exactly one final notification reporting how the command ended — success, failure, stop, or timeout. Notifications are capped per monitor; the full retained output is always available with `read`.
