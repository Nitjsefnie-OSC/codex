    ...
    fn handle(&self, invocation: ToolInvocation) -> codex_tools::ToolExecutorFuture<'_> {
        Box::pin(async move {
            ...
            let context_window = match server_reported_model.as_ref() {
                Some(_) if reported_model_has_catalog_metadata => reported_model_info
                    .as_ref()
                    .and_then(|model_info| model_info.context_window),
                Some(_) => None,
                None => requested_context_window,
            };
            let display_name_provenance =
                if server_reported_model.is_none() || reported_model_has_catalog_metadata {
                    MODEL_CATALOG_PROVENANCE
                } else {
                    SERVER_MODEL_SLUG_PROVENANCE
                };
            let context_window_provenance =
                if server_reported_model.is_none() || reported_model_has_catalog_metadata {
                    MODEL_CATALOG_PROVENANCE
                } else {
