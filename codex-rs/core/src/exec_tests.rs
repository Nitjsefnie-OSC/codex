...
/// Drain every `ExecCommandOutputDelta` payload the exec published for `stream`.
fn drain_delta_chunks(
...
#[cfg(unix)]
#[tokio::test]
async fn line_chunking_streams_each_line_as_its_own_delta() -> Result<()> {
    ...
    let params = ExecParams {
        command: vec![
            "/bin/sh".to_string(),
            "-c".to_string(),
            "echo alpha; sleep 0.3; echo beta; sleep 0.3; printf 'no-newline'".to_string(),
        ],
        cwd: cwd.clone(),
    ...
    // Each line is its own event, and the trailing partial line is flushed at EOF
    // rather than being swallowed.
    assert_eq!(
        drain_delta_chunks(&rx_event, ExecOutputStream::Stdout),
        vec![
            b"alpha\n".to_vec(),
            b"beta\n".to_vec(),
            b"no-newline".to_vec(),
...
#[cfg(unix)]
#[tokio::test]
async fn byte_chunking_remains_the_default_shape_for_shell_execs() -> Result<()> {
    let (tx_event, rx_event) = async_channel::unbounded();
    let cwd = codex_utils_absolute_path::AbsolutePathBuf::current_dir()?;
    let params = ExecParams {
        command: vec![
    ...
    assert_eq!(output.exit_code, 0);
    // Byte mode publishes output that has no newline at all, unchanged.
    assert_eq!(
        drain_delta_chunks(&rx_event, ExecOutputStream::Stdout),
