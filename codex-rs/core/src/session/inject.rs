                ...
                match input {
                    TurnInput::ResponseItem(item) => {
                        self.record_annotated_conversation_items(
                            turn_context.as_ref(),
                            turn_context.as_ref().model_info(),
                            vec![item],
                        )
                        .await;
                    }
            ...
            match input {
                TurnInput::ResponseItem(item) => {
                    self.record_annotated_conversation_items(
                        ...
                        fallback_turn_context.model_info(),
                        vec![item],
                    )
                    .await;
                }
...
/// Fork: a batch of persisted background inputs that may require waking an
/// idle parent, carrying the provenance needed to publish that wake.
