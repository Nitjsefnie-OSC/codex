...
async fn active_completion_preserves_earlier_mailbox_order() {
    ...
    .await;
    sess.input_queue
        .enqueue_mailbox_communication(progress.clone(), /*parent_turn_id*/ None)
        .await;
...
async fn deferred_mailbox_precedes_later_injected_context() {
    ...
    assert_eq!(
        ...
        vec![
            TurnInput::InterAgentCommunication(progress),
            TurnInput::ResponseItem(injected),
        ]
    );
    sess.abort_all_tasks(TurnAbortReason::Replaced).await;
}
...
async fn idle_mailbox_precedes_later_history_injection() {
    ...
    sess.input_queue
        .enqueue_mailbox_communication(progress, /*parent_turn_id*/ None)
        .await;
...
async fn non_regular_task_preserves_older_trigger_mail_wake() {
    ...
    sess.input_queue
        .enqueue_mailbox_communication(trigger, Some("parent-turn".to_string()))
        .await;
