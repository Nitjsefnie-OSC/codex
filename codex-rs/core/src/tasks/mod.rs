...
use crate::session::PendingParentTurn;
...
impl Session {
    ...
    async fn start_task_inner<T: SessionTask>(
        ...
        let background_delivery_guard = self
            ...
            .await;
        let (mut pending_items, parent_turn) = if preserve_pending_input {
            ...
            match self
                ...
                .await
            {
                PendingInputBatch::Foreground(items) => (items, PendingParentTurn::Empty),
                PendingInputBatch::Background { items, parent_turn } => (items, parent_turn),
            }
        };
        let mut input = input;
        if task_kind == TaskKind::Regular && !input.is_empty() && !pending_items.is_empty() {
            ...
            let requests_background_wake = pending_items.iter().any(|input| {
                matches!(input, TurnInput::AgentCompletion(_))
                    ...
                    || matches!(input, TurnInput::ResponseItem(item) if crate::context::is_background_notification(item))
            });
            let publishes_agent_completion_activity = pending_items.iter().any(|input| {
                matches!(input, TurnInput::AgentCompletion(_))
                    || matches!(input, TurnInput::ResponseItem(item) if crate::context::SubagentNotification::is_response_item(item))
            });
        ...
        if matches!(
            mailbox_parent_provenance,
            MailboxParentProvenance::Attribute
        ) && let PendingParentTurn::Unique(id) = &parent_turn
        {
            turn_context
                .turn_metadata_state
