...
async fn yielded_exec_completion_wakes_idle_session_once_and_remains_pollable() -> Result<()> {
    ...
    let completion_gate = test.workspace_path("yielded-exec-completion.ready");
    let completion_gate = shlex::try_join([completion_gate.to_string_lossy().as_ref()])?;
    let command = format!(
    ...
    fs::write(
        test.workspace_path("yielded-exec-completion.ready"),
        b"ready",
    )?;
