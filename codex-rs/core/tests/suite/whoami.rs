...
async fn whoami_reports_server_routed_model_and_request_provenance() -> Result<()> {
    ...
    assert_eq!(
        output["display_name_provenance"],
    ...
    assert_eq!(
        output["context_window_provenance"],
        "model_catalog_configuration"
    );
...
async fn whoami_does_not_reuse_server_identity_across_sampling_steps() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let first =
        whoami_response("response-1", "whoami-call-1").insert_header("OpenAI-Model", SERVER_MODEL);
    ...
    assert_eq!(
        output["model_identity_provenance"],
        "request_metadata_unverified"
    );
...
async fn whoami_does_not_attest_websocket_handshake_model() -> Result<()> {
    ...
    assert_eq!(
        connections.len(),
        2,
        "expected warmup and real-turn connections"
    );
    let connection = &connections[1];
    assert_eq!(
        connection.len(),
        3,
        "expected two tool calls and continuation"
    );
    let first_output = connection[1].body_json()["input"]
        .as_array()
        .and_then(|items| {
            items.iter().find(|item| {
                item["type"] == "function_call_output" && item["call_id"] == "ws-whoami-call-1"
            })
        })
    ...
    assert_eq!(
        first_output["model_identity_provenance"],
        "request_metadata_unverified"
    );
    assert_eq!(first_output["model_identity_verified"], false);

    let second_output = connection[2].body_json()["input"]
        .as_array()
        .and_then(|items| {
            items.iter().find(|item| {
                item["type"] == "function_call_output" && item["call_id"] == "ws-whoami-call-2"
            })
        })
        .and_then(|item| item["output"].as_str())
        .map(serde_json::from_str::<Value>)
    ...
    assert_eq!(
        second_output["model_identity_provenance"],
        "request_metadata_unverified"
    );
...
async fn whoami_retry_clears_stale_model_in_the_same_sampling_step() -> Result<()> {
    ...
    let response_mock =
        mount_response_sequence(&server, vec![first_attempt, second_attempt, continuation]).await;
